...
//ATTENTION, il est interdit d'inclure un autre fichier ici. On se démerde intégralement avec nous même.
#include "stm32g4xx.h"
#include "bootloader.h"



//Les variables static sont également interdites !
//Toutes les fonctions de ce fichier devront être placées dans la section .bootloader (à l'aide de la macro bl_func !)
...
__attribute__((section(".bootloader.begin"))) void bootloader(uint32_t version_of_toaster)
{
	...
	//on envoie un message à l'TOASTER pour demander si un programme nous attend.
	TOASTER_send_request_for_program();

	//on attend une réponse pendant 10ms
	uint8_t res;
	res = TOASTER_receive(&msg, NULL, 40000000);//40000);	//fonction blocante avec timeout.
	//TODO réduire le timeout lorsque le TOASTER sera correctement implémenté...
	...
	if(res == 1 && msg.sid == SID_BOOTLOADER_PROGRAM_AVAILABLE && msg.size >= 8)
	{
		...
		return;    //pas de programme à charger.

	if(version_of_toaster == toaster_version_available)
		return;	//on dispose déjà de la version qui correspond à notre bluepill toaster... --> on rejoint le programme.

	if (program_size > 120*1024)
		return;	//programme trop gros, on ne peut pas le charger.

	Unlock();
	//Erase la flash (en fonction de la taille du programme à recevoir)
	BL_FLASH_Erase(program_size);

	FLASH_Status status;
	packet_t packet;
	for(int p = nb_packets-1; p>=0; )	//pas de p-- ici, on décrémente qu'après vérification de validité du packet.
	{
		//demander le packet (en partant du dernier, pour terminer par le début, moins risqué en cas de coupure de communication ou de courant imminente)
		TOASTER_ask_for_packet(p);

		//réception du packet
		if(TOASTER_receive(&msg, &packet, 400000000) && msg.sid == SID_TOASTER_PACKET)
		{	//Si ok :
			//écrire le packet dans la flash
			uint32_t * address;
	...
	while((USART1->ISR & USART_ISR_TC) == 0);	//attendre que la transmission soit terminée !
	RCC->APB2ENR = 0;
	RCC->APB1ENR1 &= ~(1<<28);
	CLEAR_BIT(RCC->AHB2ENR, RCC_AHB2ENR_GPIOAEN);	//GPIOA CLK OFF
	CLEAR_BIT(RCC->APB2ENR, RCC_APB2ENR_USART1EN); 	//USART1 CLK OFF
...
static void BL_FLASH_Erase(uint32_t program_size)
{
	...
	//Les secteurs 0 et 1 ne sont pas effacés, ils contiennent le startup et le bootloader.
		//le startup sera effacé au dernier moment, lors de son écrasement, pour ne pas risquer de perdre le lien vers le bootloader.
		//le bootloader n'est jamais écrasé par le bootloader lui-même...
	//on efface les suivants si nécessaires selon l'adresse de fin du programme.
...
//		    FLASH_FlushCaches();
	...
	//	FLASH->CR |= FLASH_PSIZE_WORD;
		...
		for(uint32_t i = 0; i<packet->size/8; i++)
		{
			...
			if (a < (uint32_t *)0x08000800 || a >= (uint32_t *)0x08001000)	//on écrase pas le secteur 1 (bootloader)!
			{
