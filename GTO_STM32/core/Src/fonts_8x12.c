...
#include "fonts.h"
#include "st7789v.h"  // pour ST7789_DrawPixel

static const uint8_t font_8x12_data[]= {
		 ...
		 // déclaration si tableau séparé
};

FontDef Font_8x12 = {
    .data = font_8x12_data, // ou simplement `font_8x12` si tu déclares inline
    .width = 8,
    .height = 12
};

void ST7789_WriteChar_8x12(char ch, FontDef font, uint16_t x, uint16_t y, uint16_t color, uint16_t bgcolor)
{
    if (ch < 32 || ch > 126) return;

    uint16_t row, col;
    const uint8_t* glyph = &font.data[(ch - 32) * font.height]; // chaque caractère = font.height octets (1 ligne par octet)
...
void ST7789_WriteString_8x12(char* str, FontDef font, uint16_t x, uint16_t y, uint16_t color, uint16_t bgcolor)
{
    while (*str) {
        ST7789_WriteChar(*str, font, x, y, color, bgcolor);
        x += font.width + 2; // +1 pixel d’espace entre caractères
        str++;
    }
}
